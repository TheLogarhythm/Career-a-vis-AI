...
const Task2 = () => {
  ...
  useEffect(() => {
    if (selectedPoint && mode === 'timeflow') {
      const currentData = data.find(d => 
        d.industry === selectedPoint.industry && d.year === currentYear
      );
  ...
  const drawTimeflowMode = (g) => {
    ...
    if (prevYear >= d3.min(data, d => d.year)) {
      g.selectAll(".prev-circle")
        ...
        .join("circle").attr("class", "prev-circle")
        .attr("cx", d => scales.x(d.avgAIIntensity)).attr("cy", d => scales.y(d.avgSalary))
        .attr("r", d => scales.size(d.jobCount)).attr("fill", "none")
        .attr("stroke", d => INDUSTRY_COLORS[d.industry]).attr("stroke-width", 2)
        ...
        .on("mouseenter", (e, d) => setHoveredPoint(d)).on("mouseleave", () => setHoveredPoint(null));
    }

    g.selectAll(".current-circle")
      .data(currentData.filter(d => selectedIndustries.size === 0 || selectedIndustries.has(d.industry)))
      .join("circle").attr("class", "current-circle")
      .attr("cx", d => scales.x(d.avgAIIntensity)).attr("cy", d => scales.y(d.avgSalary))
      .attr("r", d => scales.size(d.jobCount)).attr("fill", d => INDUSTRY_COLORS[d.industry])
    ...
    if (selectedPoint) {
      ...
      if (targetData) {
        g.append("line").attr("class", "connection-line")
          .attr("x1", scales.x(selectedPoint.avgAIIntensity)).attr("y1", scales.y(selectedPoint.avgSalary))
          .attr("x2", scales.x(targetData.avgAIIntensity)).attr("y2", scales.y(targetData.avgSalary))
          .attr("stroke", INDUSTRY_COLORS[selectedPoint.industry]).attr("stroke-width", 2.5).attr("stroke-dasharray", "5,5");
        g.append("circle")
          .attr("cx", scales.x(targetData.avgAIIntensity)).attr("cy", scales.y(targetData.avgSalary))
          .attr("r", scales.size(targetData.jobCount) + 4).attr("fill", "none").attr("stroke", "#fff").attr("stroke-width", 2);
      }
    }
  };
  ...
  const drawCompareMode = (g) => {
    ...
    g.selectAll(".compare-right")
      .data(filteredRight).join("circle").attr("class", "compare-right")
      .attr("cx", d => scales.x(d.avgAIIntensity)).attr("cy", d => scales.y(d.avgSalary))
      .attr("r", d => scales.size(d.jobCount)).attr("fill", "none")
      .attr("stroke", d => INDUSTRY_COLORS[d.industry]).attr("stroke-width", 2)
    ...
    filteredLeft.forEach(left => {
      ...
      if (right) {
        g.append("line")
          .attr("x1", scales.x(left.avgAIIntensity)).attr("y1", scales.y(left.avgSalary))
          .attr("x2", scales.x(right.avgAIIntensity)).attr("y2", scales.y(right.avgSalary))
          .attr("stroke", INDUSTRY_COLORS[left.industry]).attr("stroke-width", 2)
          .attr("opacity", 0.6).attr("marker-end", "url(#arrowhead)");
      }
    });

    g.selectAll(".compare-left")
      .data(filteredLeft).join("circle").attr("class", "compare-left")
      .attr("cx", d => scales.x(d.avgAIIntensity)).attr("cy", d => scales.y(d.avgSalary))
      .attr("r", d => scales.size(d.jobCount)).attr("fill", d => INDUSTRY_COLORS[d.industry])
  ...
  const drawLegend = (svg) => {
    ...
    industries.forEach((industry, i) => {
      const row = legendG.append("g").attr("transform", `translate(0, ${i * 26})`)
        .style("cursor", "pointer").on("click", () => toggleIndustry(industry));
      row.append("rect").attr("width", 14).attr("height", 14).attr("rx", 3)
        ...
        .attr("stroke", selectedIndustries.has(industry) ? "#1e293b" : "none").attr("stroke-width", 2);
      ...
      industries.forEach(industry => {
        ...
        for (let i = 0; i < count; i++) {
          data.push({
            ...
            posting_year: year, industry: industry,
  ...
  return (
    <div className="task2-container">
      ...
      <div className="control-bar">
        ...
        <button className="help-btn" onClick={() => setShowGuide(true)}>❓ How to Use</button>
      </div>

      {/* 图表区域 */}
      <div className="chart-wrapper">
        <div className="chart-container">
          <svg ref={svgRef} width={WIDTH + MARGIN.left + MARGIN.right} height={HEIGHT + MARGIN.top + MARGIN.bottom} onClick={handleSvgClick} />
      ...
      <div className="timeline-section">
        {mode === 'timeflow' && (
          <button className={`play-button-cool ${isPlaying ? 'playing' : ''}`} onClick={() => setIsPlaying(!isPlaying)}>
            ...
            <div className="pulse-ring"></div><div className="pulse-ring delay"></div>
          </button>
        )}
              ...
              <div className="clock-draggable" style={{ left: `${((currentYear - yearRange[0]) / (yearRange[1] - yearRange[0])) * 100}%` }}
                ...
                <div className="clock-icon-animated">
                  <svg viewBox="0 0 32 32" width="32" height="32">
                    ...
                    {[...Array(12)].map((_, i) => (
                      <line key={i} x1="16" y1="4" x2="16" y2="6" stroke="#fff" strokeWidth="1" transform={`rotate(${i * 30} 16 16)`}/>
                    ))}
                  </svg>
                </div>
                ...
                <div className="compare-handle left" style={{ left: `${((compareYears.left - yearRange[0]) / (yearRange[1] - yearRange[0])) * 100}%` }}
                  onMouseDown={(e) => {
                    ...
                    const handleDrag = (moveEvent) => {
                      ...
                      setCompareYears(prev => ({ ...prev, left: Math.max(yearRange[0], Math.min(yearRange[1], year)) }));
                    };
                    const stopDrag = () => {
                      document.removeEventListener('mousemove', handleDrag);
                      document.removeEventListener('mouseup', stopDrag);
                ...
                <div className="compare-handle right" style={{ left: `${((compareYears.right - yearRange[0]) / (yearRange[1] - yearRange[0])) * 100}%` }}
                  onMouseDown={(e) => {
                    ...
                    const handleDrag = (moveEvent) => {
                      const x = Math.max(0, Math.min(rect.width, moveEvent.clientX - rect.left));
                      const year = Math.round(yearRange[0] + (x / rect.width) * (yearRange[1] - yearRange[0]));
                      setCompareYears(prev => ({ ...prev, right: Math.max(yearRange[0], Math.min(yearRange[1], year)) }));
                    };
                    const stopDrag = () => {
                      document.removeEventListener('mousemove', handleDrag);
                      document.removeEventListener('mouseup', stopDrag);
