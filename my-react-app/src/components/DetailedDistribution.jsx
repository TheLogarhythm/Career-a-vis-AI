  ...
  const calculateStats = () => {
    switch (metric) {
      ...
      case "ai_intensity":
        ...
        // 每10%一个间隔: 0-10%, 10-20%, ..., 90-100%
        const bins = [
          { range: "0-10%", count: intensities.filter((v) => v < 0.1).length },
          { range: "10-20%", count: intensities.filter((v) => v >= 0.1 && v < 0.2).length },
          { range: "20-30%", count: intensities.filter((v) => v >= 0.2 && v < 0.3).length },
          { range: "30-40%", count: intensities.filter((v) => v >= 0.3 && v < 0.4).length },
          { range: "40-50%", count: intensities.filter((v) => v >= 0.4 && v < 0.5).length },
          { range: "50-60%", count: intensities.filter((v) => v >= 0.5 && v < 0.6).length },
          { range: "60-70%", count: intensities.filter((v) => v >= 0.6 && v < 0.7).length },
          { range: "70-80%", count: intensities.filter((v) => v >= 0.7 && v < 0.8).length },
      ...
      case "automation_risk":
        ...
        // 分10个间隔，每0.1为一个间隔（0-0.1, 0.1-0.2, ... 0.9-1.0）
        const riskBins = [
          { range: "0.0-0.1", count: risks.filter((v) => v >= 0.0 && v < 0.1).length },
          { range: "0.1-0.2", count: risks.filter((v) => v >= 0.1 && v < 0.2).length },
          { range: "0.2-0.3", count: risks.filter((v) => v >= 0.2 && v < 0.3).length },
          { range: "0.3-0.4", count: risks.filter((v) => v >= 0.3 && v < 0.4).length },
          { range: "0.4-0.5", count: risks.filter((v) => v >= 0.4 && v < 0.5).length },
          { range: "0.5-0.6", count: risks.filter((v) => v >= 0.5 && v < 0.6).length },
          { range: "0.6-0.7", count: risks.filter((v) => v >= 0.6 && v < 0.7).length },
          { range: "0.7-0.8", count: risks.filter((v) => v >= 0.7 && v < 0.8).length },
